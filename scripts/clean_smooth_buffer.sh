...
FINAL_DIR="$OUTPUT_DIR/${INPUT_BASENAME}_exploded_3pass_smooth_3m_buffer"
...
# Initialize GRASS
LOCATION="$HOME/grassdata/${INPUT_BASENAME}_exploded_3pass_smooth"
...
# Step 1: Explode polygons and clean them
echo "Exploding polygons..."
v.explode input=raw output=exploded_polygons --overwrite

# Clean topology: remove duplicates, dangles, small artifacts
echo "Cleaning polygons..."
v.clean input=exploded_polygons output=cleaned_polygons tool=rmdupl,rmdangle,rmbridge,rmarea --overwrite

# Snap vertices to avoid misalignment and precision issues (Threshold: 0.01)
echo "Snapping vertices..."
...
# Step 2: Three-pass smoothing - run v.generalize three times in sequence
...
v.generalize input=smooth_pass2 output=smooth_final \
    method=chaiken \
    iterations=2 \
    threshold=3 \
    --overwrite
...
v.out.ogr input=smooth_buffer_clean \
    output="$FINAL_DIR/${INPUT_BASENAME}_exploded_3pass_smooth_3m_buffer_clean.geojson" \
    format=GeoJSON \
    --overwrite
...
v.out.ogr input=smooth_final \
    output="$FINAL_DIR/${INPUT_BASENAME}_pass3_final.geojson" \
    format=GeoJSON \
    --overwrite
...
echo ""
echo "=== 3-PASS SMOOTH + 2.5M BUFFER COMPLETE ==="
echo "Final file created: $FINAL_DIR/${INPUT_BASENAME}_exploded_3pass_smooth_3m_buffer_clean.geojson"
echo ""
...
echo "  - Pass 3: $FINAL_DIR/${INPUT_BASENAME}_pass3_final.geojson"
echo ""
