...
FINAL_DIR="$OUTPUT_DIR/${INPUT_BASENAME}_smooth_2m_buffer"
...
# Step 2: Buffer operations (2m buffer)
echo "Step 2: Buffering with 2m buffer..."
v.buffer input=light_smooth output=light_buffered_out distance=2.0 --overwrite
...
v.out.ogr input=light_smooth_buffer_clean \
    output="$FINAL_DIR/${INPUT_BASENAME}_smooth_2m_buffer_clean.geojson" \
    format=GeoJSON \
    --overwrite
EOF

echo ""
echo "=== SMOOTH + 2M BUFFER COMPLETE ==="
echo "Final file created: $FINAL_DIR/${INPUT_BASENAME}_smooth_2m_buffer_clean.geojson"
echo ""
